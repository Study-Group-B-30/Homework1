...
```{r, gross_on_fblikes}
ggplot(movies_cleaned,
       aes(x=gross,y=cast_facebook_likes))+
  geom_point()+
  labs(title= "Gross Profit and Cast Facebook Likes",
       x= "Gross Profit",
...
ggplot(movies_cleaned,
  ...
  geom_point()+
  labs(title= "Gross Profit and Budget",
       x= "Gross Profit",
       y= "Budget")
...
nyse %>% 
  group_by(sector) %>% 
  summarise(number_companies=n_distinct(name)) %>% 
  arrange(desc(number_companies))
```
